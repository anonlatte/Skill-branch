...
class SearchBgHelper(
    ...
    constructor(context: Context, focusListener: (top: Int, bottom: Int) -> Unit) : this(
        context,
        focusListener,
        null,
        null,
    ...
    private val multiLineRender: SearchBgRender = MultiLineRender(
    ...
    fun draw(canvas: Canvas, text: Spanned, layout: Layout) {
        ...
        spans.forEach {
            ...
            if (headerSpans.isNotEmpty()) {
                headerSpans[0].run {
                    this@SearchBgHelper.topExtraPadding = if (spanStart in firstLineBounds ||
                        spanEnd in firstLineBounds
                    ) {
                        topExtraPadding
                    } else {
                        0
                    }
                    this@SearchBgHelper.bottomExtraPadding = if (spanStart in lastLineBounds ||
                        spanEnd in lastLineBounds
                    ) {
                        bottomExtraPadding
                    } else {
                        0
...
abstract class SearchBgRender(val padding: Int) {
    ...
    fun getLineTop(layout: Layout, line: Int) = layout.getLineTopWithoutPadding(line)
    fun getLineBottom(layout: Layout, line: Int) = layout.getLineBottomWithoutPadding(line)
}
