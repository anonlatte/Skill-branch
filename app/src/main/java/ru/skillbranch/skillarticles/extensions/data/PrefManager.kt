...
class PrefManager(context: Context = App.applicationContext()) {
    val settings: LiveData<AppSettings>
        get() {
            val isBig = dataStore.data.map {
                it[booleanPreferencesKey(this::isBigText.name)] ?: false
            }
            val isDark = dataStore.data.map {
                it[booleanPreferencesKey(this::isDarkMode.name)] ?: false
            }
            return isDark.zip(isBig) { dark, big ->
                AppSettings(isDarkMode = dark, isBigText = big)
            }
                .onEach { Log.e(javaClass.canonicalName, "settings $it") }
